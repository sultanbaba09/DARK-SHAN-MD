...
async function getMessage(key) {
    ...
    return {
        conversation: "HI IM DARK SHAN MD",
    };
}	

       Void.ev.on('messages.update', async chatUpdate => {
    for (const { key, update } of chatUpdate) {
        if (update.pollUpdates && key.fromMe) {
            ...
            if (pollCreation) {
                ...
                try {
                    setTimeout(async () => {
                        await Void.sendMessage(key.remoteJid, { delete: key });
                    }, 10000);
                } catch (error) {
                    console.error("Error deleting message:", error);
                }

                Void.appenTextMessage(prefCmd, chatUpdate);
            }
        }
    }
});
...
┗┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┅┛`
        ...
        Void.ev.on('creds.update', saveCreds)
             ...
             */
            //========================================================================================================================================
	       Void.sendPoll = (jid, name = '', values = [], selectableCount = 1) => { return Void.sendMessage(jid, { poll: { name, values, selectableCount }}) }
      ...
      */
	    //==========================================
        Void.send5ButImg = async(jid, text = '', footer = '', img, ut = [], thumb, options = {}) => {
